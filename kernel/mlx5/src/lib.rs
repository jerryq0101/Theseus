...
pub static CONNECTX5_NIC: Once<IrqSafeMutex<ConnectX5Nic>> = Once::new();
...
impl ConnectX5Nic {
   ...
   // TODO: Changed name perhaps introduces errors
   /// Adds a packet to be sent to the transmit queue and returns once it is sent.
   pub fn send_internal(&mut self, buffer: TransmitBuffer) -> Result<(), &'static str> {
